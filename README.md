...
This project is configured to use the custom domain `guillermoyoga.com` with GitHub Pages. The domain configuration is already set up in the repository, but you'll need to configure your DNS settings.

### DNS Configuration Required

To complete the custom domain setup, configure the following DNS records with your domain registrar:

#### For Apex Domain (guillermoyoga.com):
...
Add a CNAME record:
```
www.guillermoyoga.com -> guillermoyoga.com
```
...
3. Ensure "Custom domain" is set to `guillermoyoga.com`
4. Enable "Enforce HTTPS" once DNS propagation is complete (may take 24-48 hours)

**Note**: DNS changes can take up to 48 hours to propagate globally. GitHub will automatically provision an SSL certificate once the DNS is properly configured.

## Installation & Setup

### Using Docker (Recommended for Development)

The easiest way to get started with local development is using Docker:
...
The site will automatically reload when you make changes to your files thanks to Jekyll's live reload feature.
...
- **Rebuild the container** (after changing dependencies): `docker-compose up --build`
- **Run Jekyll commands**: `docker-compose exec jekyll bundle exec jekyll [command]`
- **Install new gems**: `docker-compose exec jekyll bundle install`
- **Run npm commands**: `docker-compose run --rm npm npm [command]`
